...
const LifeIssuing = () =>{
    ...
    const delIssuing = (data) =>{
        axios({method:'POST',url:OnRun+'/issuing/delfilelife',data:{cookie:LginKy,data:data}
        }).then(response=>{
            ...
            columns:[
                ...
                {title:"حذف", field:"", hozAlign:'center',headerHozAlign:'center',resizable:true, widthGrow:2,
                    cellClick:function(e, cell){
                        ...
                        setDataDel({data:data,msg:'ایا از حذف "'+data.comp+'" مطمعن هستید؟'})
    ...
    const uploadFile = () =>{
        ...
        }else if(Comp==''){setMsg('لطفا شرکت بیمه را انتخاب کنید')
        }else{
    ...
    const GetDfIssuing = () =>{
        axios({method:'POST',url:OnRun+'/issuing/getdflife',data:{cookie:LginKy}
        }).then(response=>{
            if(response.data.replay){
            ...
            {AddElement?
                <div className="FormPlus">
                    <div className="FldPlus">
                        ...
                        <label>
                            <p>نمونه فایل</p>
                            <button><a href='/sample/issuingLife.xlsx' download="issuingLife.xlsx">نمونه فایل</a></button>
                        </label>
                   </div>
                   <div className="BtnBx">
