...
const CustomerRatingsFee = () =>{
    ...
    if(df!=null){
        var table = new Tabulator("#data-table", {
            ...
            columns:[
                ...
                {title:"بیمه گر", field:"comp", hozAlign:'center',headerHozAlign:'center',resizable:true, widthGrow:6,headerFilter:"input"},
                {title:"گروه رشته", field:"groupMain", hozAlign:'center',headerHozAlign:'center',resizable:true, widthGrow:6,headerFilter:"input"},
                {title:"کارمزد", field:"كارمزد قابل پرداخت", hozAlign:'center',headerHozAlign:'center',resizable:true, widthGrow:4,headerFilter:"input",bottomCalc:'sum',formatter:function(cell){return(cell.getValue().toLocaleString())},bottomCalcFormatter:function(cell){return(cell.getValue().toLocaleString())},formatter:function(cell){return(cell.getValue().toLocaleString())}},                
    ...
    const GetDf = () =>{
        if(consultant!=null && Date!=null){
            setLoaderAct(true)
            axios({method:'POST',url:OnRun+'/reports/customerratingsfee',data:{cookie:LginKy,consultant:consultant,Date:Date}
            }).then(response=>{
...
export default CustomerRatingsFee